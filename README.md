# Publications
@inproceedings{sboui2017_3d,
  author    = {Sboui, Ilhem and Jribi, Majdi and Ghorbel, Faouzi},
  title     = {3D Human Shapes Correspondence Using the Principal Curvature Fields 
               on a Local Surface Parameterization},
  booktitle = {Proceedings of the 12th International Joint Conference on 
               Computer Vision, Imaging and Computer Graphics Theory 
               and Applications (VISAPP)},
  year      = {2017},
  address   = {Portugal}
}


@inproceedings{sboui2016_accurate,
  author    = {Sboui, Ilhem and Jribi, Majdi and Ghorbel, Faouzi},
  title     = {Accurate 3D Shape Correspondence by a Local Description 
               Darcyan Principal Curvature Fields},
  booktitle = {Reconnaissance des Formes et Intelligence Artificielle (RFMI)},
  year      = {2016},
  address   = {Tunisia},
  note      = {IAPR Best Student Paper Prize 2016}
}


@inproceedings{sboui2015_geodesic,
  author    = {Sboui, Ilhem and Hajji, Mehdi and 
               {Bel Hadj Khelifa}, Wafa and Ghorbel, Faouzi},
  title     = {Geodesic Local Analysis of 3D Dynamic Objects},
  booktitle = {Reconnaissance des Formes et Intelligence Artificielle (RFMI)},
  year      = {2015},
  address   = {Tunisia}
}


@inproceedings{sboui2013_semi,
  author    = {Sboui, Ilhem and Gadacha, Wieme and Ghorbel, Faouzi},
  title     = {Semi-Parametric Approach of Osteosynthesis Operation 
               for Proximal Femur Fractures Repair},
  booktitle = {Traitement et Analyse de l'Information: 
               Méthodes et Applications (TAIMA)},
  year      = {2013},
  address   = {Tunisia}
}
